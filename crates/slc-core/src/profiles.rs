...
impl<S: StorageBackend> ProfileManager<S> {
    ...
    fn doc(doc_id: String, doc_type: &str, content: &str, seat_id: &str) -> Document {
        ...
        Document::new(doc_id, DocumentCategory::System, content, meta, vec!["profile".into()], Some(seat_id.into()))
    }

    pub async fn get_user_profile(&self, user_id: &str, seat_id: &str) -> SlcResult<Option<String>> {
        let _ = seat_id;
        let id = Self::user_id(user_id);
        Ok(self.store.kb_get(&id).await?.map(|d| d.content))
    }

    pub async fn upsert_user_profile(&self, user_id: &str, content: &str, seat_id: &str) -> SlcResult<bool> {
        let id = Self::user_id(user_id);
    ...
    pub async fn get_seat_profile(&self, seat_id: &str) -> SlcResult<Option<(String, String)>> {
        ...
        let Some(doc) = self.store.kb_get(&id).await? else { return Ok(None) };
        let tz = doc
            .metadata
            .extra
            .get("timezone")
    ...
    pub async fn resolve_user_id(&self, seat_id: &str) -> SlcResult<String> {
        Ok(self
            ...
            .get_seat(seat_id)
            .await?
            .and_then(|s| s.metadata.get("user_id").and_then(|v| v.as_str()).map(String::from))
    ...
    async fn _all_profiles(&self, doc_type: &str, seat_id: &str) -> SlcResult<Vec<Document>> {
        ...
        let mut docs = self.store.kb_find(&filter, &crate::storage::DocSort::by_created(SortDir::Asc), 100).await?;
...
mod tests {
    ...
    fn mgr() -> (ProfileManager<std::sync::Arc<dyn StorageBackend>>, std::sync::Arc<dyn StorageBackend>) {
    ...
    async fn user_profile_create_update_read() {
        let (m, _) = mgr();
        assert!(m.get_user_profile("alice", "seat_u").await.unwrap().is_none());
        m.upsert_user_profile("alice", "Likes concise replies", "seat_u").await.unwrap();
        assert_eq!(m.get_user_profile("alice", "seat_u").await.unwrap().unwrap(), "Likes concise replies");
        m.upsert_user_profile("alice", "Likes JSON", "seat_u").await.unwrap();
        assert_eq!(m.get_user_profile("alice", "seat_u").await.unwrap().unwrap(), "Likes JSON");
    }
    ...
    async fn seat_profile_timezone() {
        ...
        m.upsert_seat_profile("seat_s", "Moscow workspace", Some("Europe/Moscow")).await.unwrap();
